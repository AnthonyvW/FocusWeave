...
/// be summed in a single pass over the gradients — 33 of OpenCV's 39 dot
/// products over six full-resolution planes collapse into one traversal. The
/// error projection needs lambda, so it waits for `project_error`.
#[derive(Clone)]
struct Normals {
